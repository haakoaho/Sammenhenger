document.addEventListener("DOMContentLoaded", () => {
  ...
  const difficultyStarsContainer = document.getElementById(
  ...
  function render() {
    ...
    state.completedGroups
      ...
      .forEach((group) => {
        ...
        groupDiv.innerHTML = `
                ...
                <div class="completed-group-items">${group.items.join(
                  ", "
                )}</div>
            `;
    ...
    deselectButton.disabled =
      state.activeItems.length === 0 || state.isFinished;
    ...
    if (matchedGroup) {
      animateCorrectGuess(() => {
        ...
        state.items = state.items.filter(
          (item) => !state.activeItems.includes(item)
        );
        state.activeItems = [];
  ...
  function endGame(lost = false) {
    ...
    if (lost) {
      const remainingGroups = state.currentPuzzle.groups.filter(
        (g) => !state.completedGroups.includes(g)
      );
  ...
  function showResults() {
    ...
    titleEl.textContent = `Resultater - ${
      resultTitles[mistakesMade] || resultTitles[4]
    }`;
    ...
    state.guesses.forEach((guess) => {
      guess.forEach((item) => {
        const group = state.currentPuzzle.groups.find((g) =>
  ...
  function isAlreadyGuessed(guess) {
    return state.guesses.some(
      (g) => JSON.stringify(g) === JSON.stringify(guess)
    );
  }

  function isOneAway() {
    return state.currentPuzzle.groups.some((group) => {
      const matches = state.activeItems.filter((item) =>
